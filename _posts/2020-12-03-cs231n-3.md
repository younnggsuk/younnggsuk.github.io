...
- Loss Function은 W를 받아 class score를 보고 정량적으로 얼마나 나쁜지를 결정하는 함수를 의미한다. 즉, 최적의 W를 결정하기 위해 필요한 함수이다.
...
- Multiclass SVM Loss (Hinge Loss)의 계산 방식은 다음과 같다.
...
- 아래의 4개 슬라이드를 통해 구체적으로 Loss를 구하는 과정을 살펴보자.
...
- 첫번째 데이터에서, 정답 class인 cat과 score를 비교하고 모두 더해준다.
...
- 두번째 데이터에서, 정답 class인 car와 score를 비교하고 모두 더해준다.
...
- 세번째 데이터에서, 정답 class인 frog와 score를 비교하고 모두 더해준다.
...
- 마지막으로, 앞에서 구한 값들의 평균을 구해서 Loss를 구한다.
...
- 위 슬라이드의 계산 결과를 보면, W를 2배로 해도 Loss는 같게 계산된다는 것을 알 수 있다.
- 따라서, Loss를 통해 구한 최적의 W는 유일하지 않다는 것을 알 수 있다.
...
- Regularization은 Model이 train data에 너무 fit해지면서, 복잡해지지 않도록 하기 위한 방법 중 하나이다. 즉, overfitting 방지 기법이다.
...
- Regularization은 Loss function에 Regularization항을 추가하는 방식으로 penalty를 주어서 모델이 간단해지도록 한다.
  - 위 슬라이드에서, $R(W)$가 추가된 regularization term을 의미
  - $\lambda$는 hyperparameter로서, regularization strength를 조절
...
- Optimization --> Loss가 최소가 되는 지점을 찾자!
...
- W의 값을 random하게 설정하고 Loss를 계산하는 과정을 반복하면서, 가장 최소일 때를 찾는 방법이다.
- 좋지 못한 방법이므로 절대 사용하지 말자.
  - 그냥 운에 따르는 것
...
- 위 그림과 같은 계곡에서 눈을 가린 채 가장 낮은 지점을 찾는다고 가정하고, 발로 경사가 있는 지점을 계속해서 느끼면서 해당 방향으로 나아가다 보면, 가장 낮은 지점에 도달할 수 있다.
...
- 하지만 이 방법은 너무 비효율적이다. 그냥 함수를 미분해서 loss를 계산해보자. (Analytic Gradient)
