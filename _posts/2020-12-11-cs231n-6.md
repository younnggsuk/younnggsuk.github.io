...
- sigmoid에서는 $\dfrac{\partial f_i}{\partial w_i} = x_i$ (local gradient)가 항상 양의 값이기 때문에, $\dfrac{\partial L}{\partial w_i}$는 $\dfrac{\partial L}{\partial f}$ (upstream gradient)과 항상 같은 부호를 갖게 된다.
